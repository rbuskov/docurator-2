...
- Gmail OAuth flow and account-add UI → Slice 002
- Reading Gmail messages or any Gmail API client wrapper → Slice 003
- SQLite plumbing (DB connection, migration runner, the `accounts` table, `./data` bind mount) → Slice 002
...
- **docker-compose.yml.** Single `app` service. Maps `3737:3737`. Includes `extra_hosts: host.docker.internal:host-gateway` so later slices that talk to host-side Ollama work on Linux without further changes — declared now so the file is stable. **No bind-mounted volumes yet** (`./data` arrives in Slice 2 alongside the SQLite connection, and `./invoices` in Slice 6 alongside the file store, each when there's something to write). **No environment variables yet** beyond what `APP_PORT` defaults to internally.
...
- `git status` on a fresh clone after `docker compose up --build` reports a clean working tree (i.e. `.gitignore` covers everything generated by the build, including `node_modules/` and `dist/` if they leak out of the container, and any `.env` the developer creates locally).
...
- **Tailwind not wired yet.** The `tailwindcss` package is declared in `package.json` so the dependency is in place, but no Tailwind classes or config are added. Slice 002's Dashboard ships unstyled (default browser CSS plus minimal inline layout); Tailwind/shadcn wiring is deferred to Slice 016 polish, matching the Out of scope above.
