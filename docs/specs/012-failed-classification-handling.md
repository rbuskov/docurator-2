...
- **`countUnresolvedFailures` query cost.** Runs the latest-attempt-per-message subquery over `processed_messages`. Fine at the scale of a few thousand messages; if a future install grows past tens of thousands of rows and queries slow, the right answer is to materialize a `latest_attempt_id` column or a view.
- **Retry vs. Reclassify split.** Failed rows show a warning-styled "Retry"; successful rows show "Reclassify". Both hit the same endpoint. The labels match user intent.
- **Ollama timeouts during retry.** Reuses `OLLAMA_TIMEOUT_MS` (default 120s). Closing the tab aborts the request server-side (the AbortController in `src/server/classify/ollama.ts`); a cancelled retry writes nothing — the original failed row stays the unresolved-state-of-record.
- **Bulk retry deferred to Slice 014.** Per-row Retry plus the `?status=failed` filter is the v1 surface; Slice 014's batch reclassification covers many-failures-at-once.
- **Truncated `error_message` in tooltip.** Pill tooltip caps at 200 chars; the popover shows the full text. The DB stores the full message regardless.
- **No status indicator on Inbox or Review.** Failed rows produce no `documents` rows, so they don't appear in those views. The Audit view remains the canonical failure surface, consistent with the architecture's "audit is the safety net" framing.
