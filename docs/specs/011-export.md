...
- **Most-recent `processed_messages` JOIN.** `selectDocumentsForExport` uses a subquery to take the most-recent `processed_messages` row per `(account_id, message_id)`, side-stepping the JOIN cardinality issue from the append-only audit log. Slice 006's Inbox listing and Slice 007's Review queue use the same JOIN pattern; until they adopt the most-recent-attempt subquery, reclassified documents may show duplicate rows in those listings. Slice 016's out-of-scope list flags this for follow-up cleanup.
