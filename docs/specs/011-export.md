...
- **Files / modules:**
  ...
  - `src/server/export/manifest.ts` — `buildManifest(rows): ManifestRow[]`. **Single source of truth** for the row shape consumed by both CSV and Markdown serializers. `ManifestRow` columns: `filename` (relative path within zip, see folder layout below), `account_email`, `account_label` (=`accounts.display_name` falling back to `accounts.email`), `source_email_date` (RFC 3339 from `internal_date`), `source_email_sender` (`sender_domain`), `source_email_subject`, `classification`, `confidence`, `model_used`, `vendor`, `amount`, `currency`, `transaction_date`, `tags` (semicolon-joined names, sorted), `notes` (empty string in this slice; populated by Slice 016 once the column exists), `gmail_message_id`. Includes a small comment in the source explaining that field order matters because both serializers iterate `Object.keys(row)` in the same order.
  ...
  - `src/client/views/Settings.tsx` (modified) — the previously-disabled Fiscal-year section now mounts `FiscalYearSettings`. The other disabled section (Senders) remains disabled until Slice 015 fills it.
...
- Group-aware export (a "include only the canonical sibling per group" filter, or any other surfacing of `document_groups` membership in the zip) → not planned for v1; matching Slice 013's Out of scope, the export packages the rows that match the user's filters and ignores group membership. Users who want to exclude a duplicate from the export can reject it in Review.
...
- Editing the `notes` field anywhere → Slice 016 (introduces the column, the Review-pane textarea, and the manifest population)
...
- **`notes` column emitted as empty string.** Locks the CSV/Markdown manifest schema now so Slice 016's `notes` column on `documents` ships as a data-only change.
- **Most-recent `processed_messages` JOIN.** `selectDocumentsForExport` uses a subquery to take the most-recent `processed_messages` row per `(account_id, message_id)`, side-stepping the JOIN cardinality issue from the append-only audit log. Slice 006's Inbox listing and Slice 007's Review queue use the same subquery pattern, so reclassification (Slices 010 / 014) does not multiply rows in any of these listings.
- **Tag filter is OR.** Multi-tag selection matches "any selected tag". AND semantics is a Slice 016 polish item.
- **GET-with-long-query-string for download.** Allows share-by-URL and lets the browser drive the streaming download. Within the typical single-user install size (≤5 accounts, ≤20 tags), URL length is a non-issue.
- **`archiver` store-only.** `archiver('zip', { store: true })` skips deflate so already-compressed PDFs aren't re-compressed.
- **Period-month math vs. SQLite.** `strftime('%Y-%m', ...)` plus epoch-ms-divided-by-1000 over `internal_date`. UTC is used throughout; `summary.txt` notes the basis.
- **Live-preview cost.** 200ms debounce on filter changes, plus the lightweight `COUNT(*) + SUM(amount) GROUP BY currency` query.
- **Calendar-based quarter presets.** `Q1`–`Q4` are calendar quarters regardless of fiscal-year setting. Fiscal-quarter presets can ship later.
- **Concurrent download streams.** SQLite WAL handles concurrent reads; each `archiver` instance is independent. No coordination needed.
- **Empty-result zips.** A zero-row export still produces a valid zip with header-only manifests and a `summary.txt` saying "0 documents".
