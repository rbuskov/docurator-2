...
- **Background jobs / orchestrators:**
  - Sync orchestrator (modified) — for each message, after extracting `sender_domain` and before calling the classifier:
    ...
    3. After classification, if the result is `receipt` or `invoice` AND (the sender's `listing='allow'` OR the auto-approve heuristic — see Detailed design — fires), persist the document(s) as usual but with `review_status='approved'` and `auto_approved=1`, then insert a `review_actions` row with `action='approved'` and `details = { auto: true, reason: 'allowlist' | 'trust_threshold', approved_count, rejected_count }`. The `senders.approved_count` is **not** auto-incremented for auto-approvals — the heuristic only learns from genuine user actions, avoiding runaway feedback.
...
- **Blocklist privacy property.** Blocklisted messages have their `From` header read (required for sender domain) and their `subject` stored. Body and attachments are never fetched.
- **Bulk listing changes audit volume.** A 50-domain bulk blocklist writes 50 `review_actions` rows. The audit log scales linearly with operations.
- **Auto-skipped sync events.** The `sync.message` SSE event gains a forward-compatible `kind: 'classified' | 'auto-skipped' | 'auto-approved'` discriminator so the UI can style auto-skipped progress differently.
- **Auto-approval and tags.** Auto-approved docs land with no tags. Exports filtered by tag will exclude them. Per-sender tag rules can ship later.
- **`AUTO_APPROVE_MAX_REJECTIONS=0` is strict.** One reject disables auto-approve for that sender. The threshold is env-var tunable; time-decay on rejection counts can be added later.
- **No sender renaming.** Domain changes (`stripe.com` → `payments.stripe.com`) start a fresh sender row; old stats don't transfer. The Sender Manager lets the user re-toggle listings on the new domain.
