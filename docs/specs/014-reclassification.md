...
- **In-memory diff loses on restart.** A user who runs a 4-hour reclassify and closes the browser before reading the diff loses it on the next container restart. Mitigation options: persist diffs to a `reclassify_jobs` table (extra schema), email the diff (out of architectural scope — no outbound mail), let the user export it from the UI before closing. Provisional: the audit log retains all the underlying data, so the diff is reconstructible offline. Flag.
- **`model_disagrees` semantics on clear.** Cleared on approve (user override) and on subsequent reclassify-back-to-receipt (model agrees again). Not cleared on reject (the user agreeing with the model's "not a receipt" doesn't mean the doc should hide its history). Flag for confirmation.
- **No per-attempt linkage on documents.** A document doesn't carry a pointer to the specific `processed_messages` row that produced it. For most uses (Review pane, Audit table, Export manifest) this is fine because we always pick the latest attempt. If a future audit feature wants "show me which model run produced this exact doc", a `documents.processed_message_id INTEGER REFERENCES processed_messages(id)` column would be the right addition (Slice 006's spec already flags this as a future possibility). Flag.
- **Mutex placement.** Promoting Slice 006's mutex into a shared module requires that slice's spec to evolve (or this slice's spec to be the canonical home for it). Provisional: this slice introduces `job-mutex.ts` and the Slice 006 sync orchestrator imports from it; Slice 006's spec should get a small "modified to use the shared mutex" note in a future cleanup pass. Not blocking. Flag.
- **Period filter on `internal_date`.** Choosing `internal_date` (the email's actual date) over `processed_at` is the user-intent-correct choice but means a "reclassify January 2026" run will not include messages that arrived in January but weren't synced until February. The Slice 011 export already uses similar reasoning (`transaction_date` with `internal_date` fallback); reclassify uses pure `internal_date` because it's about "what the email is" not "what its receipt is dated". Flag.
- **Concurrent reclassify against a fresh sync.** A user starts a sync, then immediately tries to reclassify; the 409 explains why. After the sync completes, the new rows are in scope for the next reclassify run. No data race; the mutex makes it impossible. Flag.
- **Long-running HTTP for `POST /api/reclassify/batch`.** The endpoint returns 202 immediately and progresses via SSE; the long-running classify happens in the background. The classify itself is per-message synchronous against Ollama (Slice 005 default 120s), and the orchestrator processes serially. A 1000-message batch on a slow CPU could take many hours; the SSE stream stays connected throughout. Flag if this becomes operationally annoying.
- **Reclassify doesn't update `documents.created_at`.** Existing docs keep their original timestamp even when the model now disagrees. The audit log captures the new attempt's `processed_at`. Acceptable; no UI surface depends on `created_at` reflecting reclassify.
- **No "preview diff" mode.** A user can't dry-run a reclassify to see what would change without actually appending rows. Architecture says reclassification is non-destructive (append-only), so the cost of doing it for real is low — but a dry-run could save Ollama compute. Flag if requested.
