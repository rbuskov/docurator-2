...
- **Files / modules:**
  ...
  - **Removed:** `src/client/views/DevSeedPanel.tsx`, `src/server/api/dev.ts` — the dev seed surface from Slice 004 is deleted in this slice. `src/server/api/processed_messages.ts` (Slice 004's `GET /api/accounts/:id/processed-messages`) is **kept** because Slice 010's Audit view will use it.
...
- **First-sync-window override.** The default 30-day window is configured via `SYNC_DEFAULT_WINDOW_DAYS` and overridable per request via the `since` field in the `POST /api/sync` body. A Dashboard date picker is part of Slice 016 polish.
