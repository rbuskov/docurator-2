...
- **Attachment fetching and rendering.** For each attachment whose MIME type is in `{image/png, image/jpeg, image/gif, image/webp, application/pdf}`, the pipeline calls `getAttachment` (read-only), decodes the base64 payload, and converts to one or more PNG buffers: images pass through; PDFs are rendered via `pdfjs-dist` + `@napi-rs/canvas`. Other MIME types are listed by name in the prompt metadata but their bytes are not sent to the model. Attachments larger than `MAX_ATTACHMENT_BYTES` (5 MB hard-coded for now) are skipped with a metadata note; this prevents one giant scan PDF from breaking inference. Both caps are documented in Risks and may be tuned in Slice 006.
...
- **`pdfjs-dist` + `@napi-rs/canvas` choice.** Renders PDFs in pure-Node without system deps, but adds tens of MB to the image. Alternatives: `mupdf-js` (smaller, WASM-based, slower init), `poppler-utils` via system install (needs Dockerfile change). Provisional: `pdfjs-dist` + `@napi-rs/canvas`. Flag for confirmation.
- **PDF page cap of 5.** Probably right for receipts, but a long invoice PDF (multi-page itemized line items) might be truncated. Architecture is silent on this; this slice picks a number to ship. Flag.
- **Attachment size cap of 5 MB.** Skips genuinely large attachments rather than risk OOMs. The metadata-only note in the prompt should keep classification correct (size + filename are usually enough to know it isn't a typical receipt). Flag.
- **Inline `cid:` images not rendered.** The body-extraction path notes their existence but does not feed them to the model in this slice. For receipts that arrive purely as a styled HTML body with embedded images (uncommon — receipts usually arrive as text + attachment, or a fully-rendered HTML where the styling is the receipt), this could miss the visual signal. Slice 006's Playwright HTML→PDF rendering subsumes this — at that point the entire styled body becomes an image the model sees. Flag.
- **Single-decision vs multi-artifact response.** The architecture allows multiple results per message; this slice returns one decision. The `artifacts` array lets a future slice (006) split decisions per artifact without changing the prompt. Flag if multi-decision should arrive earlier.
- **Synchronous endpoint with long timeout.** Holding an HTTP connection open for up to 120s is unusual for a typical web app. Acceptable for a localhost-only tool used by one person. Slice 006 will move batch work to SSE; a single classify can stay synchronous. Flag.
- **Prompt copy lives in TypeScript.** Tuning the prompt is a code change, not a data change. For a self-hosted single-user tool that's fine; if iterating on the prompt becomes frequent, consider an env-overridable file path. Flag for confirmation.
- **`format=full` payload size.** A message with a 30-MB HD photo attachment and several embedded images can blow up `getMessage`'s response. The client should set a reasonable `maxResults` and trust Gmail's API to be robust; not introducing a hard size cap here. Flag.
- **Zod adopted in this slice but not retroactively.** Slice 002 and 003 API handlers don't use `zod-validator` for their input shapes (they're trivially validated by Hono's path-param parsing). When a future slice adds richer input schemas, it will introduce them in its own scope; this slice does not retrofit earlier endpoints.
