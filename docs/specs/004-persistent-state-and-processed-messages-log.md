...
- **Append-only `processed_messages`.** No DB-level uniqueness on `(account_id, message_id)`; reclassification slices (010, 014) append additional rows for the same message. Idempotency for the dev seed here and for Slice 006's sync is enforced by an application-level `existsForMessage` check inside the same transaction as the insert. Slice 006's sync runs as a single-job mutex per account, so the check-then-insert race is moot. Slice 006's `documents` table therefore references `processed_messages` by surrogate `id`, not by composite `(account_id, message_id)`.
- **NODE_ENV detection via the server.** The client gates the dev panel on `GET /api/dev/enabled` rather than `import.meta.env.MODE`, so a production image cannot leak the panel even if `NODE_ENV` is mis-set during `vite build`.
- **Removing the dev seed panel.** Slice 006 lists `DevSeedPanel.tsx` as a removed file once the real sync ships.
- **`app_config` single-row enforcement.** `CHECK (id = 1)` plus the migration's `INSERT OR IGNORE INTO app_config(id, fiscal_year_start_month) VALUES (1, 1);` keeps the table at exactly one row.
- **Migration order vs FK enforcement.** Migrations 0002 and 0003 reference `accounts(id)` from Slice 002 and apply cleanly with `foreign_keys = ON`. Future migrations that need to alter referenced columns should disable FKs locally for that migration — standard SQLite practice.
- **WAL files on the bind mount.** WAL mode produces `app.db-wal` and `app.db-shm` sibling files inside `./data`. They're gitignored via Slice 002's `.gitignore`. The README's "Backup recommendations" section (Slice 016) covers them.
